...
export async function PUT(
  ...
  try {
    ...
    const { nama, jenis, instansi, password, email, jurusanId } = await request.json();

    console.log('📝 [PENILAI API UPDATE] Updating penilai:', id);
    console.log('📝 [PENILAI API UPDATE] Data received:', { nama, jenis, instansi, email, hasPassword: !!password, jurusanId });
    ...
    if (penilai.userId) {
      ...
      if (Object.keys(userData).length > 0) {
        console.log('🔐 [PENILAI API UPDATE] Updating user data for:', penilai.userId);
    ...
    console.log('📝 [PENILAI API UPDATE] Updating penilai with data:', updateData);
