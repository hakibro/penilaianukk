...
export default function PenilaiPage() {
	console.log(
		"🔥 [PENILAI PAGE LOADED] - Version 2.0.7",
	...
	const fetchData = async () => {
		try {
			...
			if (adminJurusan?.jurusanId) {
				...
				console.log(
					"🔍 Fetching penilai with jurusanId:",
					adminJurusan.jurusanId,
				);
			}
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(
					error.error || error.message || "Gagal menambahkan penilai",
				);
			}
	...
	const handleDelete = async (id: string, isForce: boolean = false) => {
		// Konfirmasi awal hanya untuk hapus biasa
		if (
			!isForce &&
			!confirm("Apakah Anda yakin ingin menghapus penilai ini?")
		) {
			return;
		}

		try {
			const url = `/api/admin/penilai/${id}${isForce ? "?force=true" : ""}`;
			const response = await fetch(url, { method: "DELETE" });
			const data = await response.json();

			if (!response.ok) {
				// Cek jika error dikarenakan adanya relasi data penilaian
				if (data.error === "HAS_DATA") {
					const confirmForce = confirm(
						`${data.message}\n\nPERHATIAN: Menghapus paksa akan menghilangkan semua riwayat nilai yang pernah diinput oleh penilai ini. Lanjutkan hapus paksa?`,
					);

					if (confirmForce) {
						return handleDelete(id, true); // Panggil rekursif dengan flag force=true
					}
					return; // User membatalkan hapus paksa
				}

				throw new Error(data.error || "Gagal menghapus penilai");
			}

			toast.success(
				isForce
					? "Penilai dan semua data penilaian berhasil dihapus paksa"
	...
	const handleUpdate = async (e: React.FormEvent) => {
		...
		try {
			...
			if (!response.ok) {
				const error = await response.json();
				throw new Error(
					error.error || error.message || "Gagal mengupdate penilai",
				);
			}
		...
		<div className="space-y-6">
			{adminJurusan && (
				<Alert className="bg-primary/10 border-primary/20">
					...
					<AlertDescription>
						Admin Jurusan: {adminJurusan.jurusan.nama} (
						{adminJurusan.jurusan.kode}) - Hanya menampilkan data untuk jurusan
						ini
					</AlertDescription>
				</Alert>
			)}
							...
							<div className="space-y-4 py-4">
								...
								<div className="space-y-2">
									...
									<Input
										...
										onChange={(e) => {
											console.log(
												"💻 [PENILAI] Password input changed:",
												e.target.value,
											);
									...
									/>
									<p className="text-xs text-muted-foreground">
										Password akan digunakan untuk login penilai di sistem
										penilaian
									</p>
								</div>
